...
from reactnet1 import LearnableBias, BinaryAct

class BinaryConv(nn.Conv2d): # binary -> float
    def __init__(self, *args, **kargs):
        super(BinaryConv, self).__init__(*args, **kargs)
    
    def forward(self, x):
        real_weights = self.weight
        scaling_factor = torch.mean(torch.mean(torch.mean(abs(real_weights), dim = 3, \
                            keepdim = True), dim = 2, keepdim = True), dim = 1, keepdim = True)
...
class NormalBlock(nn.Module):
    def __init__(self, channels):
        ...
        self.conv1 = BinaryConv(channels, channels, kernel_size = 3, padding = 1, bias = False)
        ...
        self.conv2 = BinaryConv(channels, channels, kernel_size = 1, bias = False)
    ...
    def forward(self, x):
        out1 = self.bn1(self.conv1(self.act(self.bias1(x))))
        out1 = x + out1
        out1 = self.bias3(self.prelu1(self.bias2(out1)))

        out2 = self.bn2(self.conv2(self.act(self.bias4(out1))))
        out2 = out1 + out2
